...
export default function ReviewerQueuePage() {
  ...
  async function handleOpenComparison(updateId) {
    setLoadingComparison(true);
    try {
      const data = await datasetsApi.getContentUpdateComparison(updateId);
      setComparisonModal({ updateId, ...data });
    } catch (err) {
      addToast(err?.response?.data?.detail || "Failed to load update comparison.", "error");
    } finally {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (results[4].status === "fulfilled") setMyReviews(myReviewsList);
      } catch {
  ...
  return (
    <DashboardShell title="Review Queue" subtitle="Evaluate pending datasets, content updates, and revision requests.">
      ...
      {/* Content Update Comparison Modal */}
      {comparisonModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 animate-fade-in" onClick={() => setComparisonModal(null)}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full p-6 max-h-[85vh] flex flex-col overflow-hidden animate-fade-in-up" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between pb-4 border-b border-border">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-bold text-navy">Content Update Comparison</h3>
                  <p className="text-xs text-gray-500">Review content diff and proposed dataset changes</p>
                </div>
              </div>
              <button type="button" onClick={() => setComparisonModal(null)} className="p-1 rounded-lg text-gray-400 hover:text-navy transition">
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="py-4 overflow-y-auto space-y-4 text-xs">
              {comparisonModal.diff_pct !== undefined && (
                <div className="flex items-center gap-3 p-3.5 rounded-xl bg-slate-50 border border-border">
                  <span className="font-semibold text-gray-500">Content Change Magnitude:</span>
                  <span className={`px-2.5 py-1 rounded-full font-bold ${
                    comparisonModal.diff_pct >= 20 ? "bg-amber-100 text-amber-800" : "bg-blue-100 text-blue-800"
                  }`}>
                    {comparisonModal.diff_pct}% diff
                  </span>
                </div>
              )}

              {comparisonModal.summary && (
                <div className="p-4 rounded-xl bg-purple-50/50 border border-purple-100 space-y-1">
                  <p className="font-bold text-purple-950 uppercase tracking-wider text-[10px]">AI / Change Summary</p>
                  <p className="text-purple-900 leading-relaxed text-xs">{comparisonModal.summary}</p>
                </div>
              )}

              {(comparisonModal.old_download_url || comparisonModal.new_download_url) && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {comparisonModal.old_download_url && (
                    <a
                      href={comparisonModal.old_download_url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="p-3 rounded-xl border border-border bg-gray-50 hover:bg-gray-100 transition flex items-center justify-between"
                    >
                      <span className="font-medium text-gray-700">Download Original File</span>
                      <ExternalLink className="w-3.5 h-3.5 text-gray-400" />
                    </a>
                  )}
                  {comparisonModal.new_download_url && (
                    <a
                      href={comparisonModal.new_download_url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="p-3 rounded-xl border border-gold/40 bg-amber-50/40 hover:bg-amber-50 transition flex items-center justify-between"
                    >
                      <span className="font-medium text-navy">Download New Version File</span>
              ...
              {comparisonModal.metadata_diff && Object.keys(comparisonModal.metadata_diff).length > 0 && (
                <div className="space-y-2">
                  <p className="font-bold text-navy uppercase tracking-wider text-[10px]">Metadata Differences</p>
                  <pre className="p-3 bg-slate-900 text-slate-100 rounded-xl overflow-x-auto text-[11px] font-mono">
                    {JSON.stringify(comparisonModal.metadata_diff, null, 2)}
                  </pre>
                </div>
              )}
            </div>
            ...
            <div className="pt-4 border-t border-border flex items-center justify-end gap-2">
              <button
                ...
                className="px-4 py-2 border border-border text-gray-700 rounded-xl text-xs font-semibold hover:bg-gray-100 transition"
              >
                Close
              </button>
              <button
                type="button"
                disabled={actionId === comparisonModal.updateId}
                onClick={() => {
                  const id = comparisonModal.updateId;
                  setComparisonModal(null);
                  handleVote("content-updates", id, "approved");
                }}
              ...
              <button
                ...
                onClick={() => {
                  const id = comparisonModal.updateId;
                  setComparisonModal(null);
                  handleVote("content-updates", id, "rejected");
                }}
...
// ── Generic Vote Table ──────────────────────────────────────────────────
function VoteTable({ type, items, emptyTitle, emptyDesc, idKey, titleKey, descKey, actionId: currentActionId, onVote, onViewComparison, loadingComparison }) {
  if (items.length === 0) {
                  ...
                  <div className="flex items-center justify-end gap-2">
                    {onViewComparison && (
                      <button
                        ...
                        onClick={() => onViewComparison(id)}
                        className="inline-flex items-center gap-1.5 text-xs font-semibold text-navy bg-slate-100 hover:bg-slate-200 rounded-lg px-3 py-2 transition-colors disabled:opacity-50"
                      >
                        <Eye className="w-3.5 h-3.5" />
                        Compare Diff
                      </button>
