import { useState } from "react";
import { Send, X } from "lucide-react";

/**
 * Research-interest picker: card grid of approved categories, multi-select
 * with removable "×" chips, plus an "add custom interest" mini-form that
 * posts a new (pending) category via onAddOtherInterest.
 */
export default function ResearchInterests({
  id,
  label,
  required = false,
  value = [],
  onChange,
  categories = [],
  onAddOtherInterest,
  disabled = false,
  readOnly = false,
}) {
  const [otherInterest, setOtherInterest] = useState("");

  const isDisabled = disabled || readOnly;

  const selectedIds = new Set(
    (Array.isArray(value) ? value : []).map((item) =>
      typeof item === "object" && item?.id ? item.id : item
    )
  );

  function toggleInterest(categoryItem) {
    if (isDisabled) return;
    const current = Array.isArray(value) ? value : [];
    const id = categoryItem.id;
    const exists = current.some(
      (item) => (typeof item === "object" && item?.id ? item.id : item) === id
    );
    if (exists) {
      onChange(current.filter((item) => (typeof item === "object" && item?.id ? item.id : item) !== id));
    } else {
      onChange([...current, categoryItem]);
    }
  }

  function removeInterest(tagToRemove) {
    if (isDisabled) return;
    const current = Array.isArray(value) ? value : [];
    const idToRemove = typeof tagToRemove === "object" && tagToRemove?.id ? tagToRemove.id : tagToRemove;
    onChange(current.filter((item) => (typeof item === "object" && item?.id ? item.id : item) !== idToRemove));
  }

  function handleAddOther() {
    if (isDisabled || !otherInterest.trim() || !onAddOtherInterest) return;
    onAddOtherInterest(otherInterest.trim());
    setOtherInterest("");
  }

  const displayItems = Array.isArray(value) ? value : [];

  return (
    <div className="mb-4">
      <div className="flex items-center justify-between mb-1">
        <label htmlFor={id} className="text-sm font-semibold text-slate-700">
          {label}
          {required && <span className="text-red-500 ml-0.5">*</span>}
        </label>
      </div>

      {/* Selected interests as removable chips */}
      <div className="flex flex-wrap items-center gap-2 mb-3">
        {displayItems.map((tag, idx) => {
          const labelText = typeof tag === "string" ? tag : tag?.name || String(tag);
          const isPending = typeof tag === "object" && tag?.pending;
          const key = (typeof tag === "object" && tag?.id) || `${labelText}-${idx}`;
          return (
            <span
              key={key}
              className={`inline-flex items-center gap-1 text-xs font-medium rounded-full px-3 py-1 ${
                isDisabled ? "bg-slate-300 text-slate-700" : "bg-[#6B5B1A] text-white"
              }`}
            >
              {labelText}
              {isPending && <span className="text-[10px] opacity-80">(pending)</span>}
              {!isDisabled && (
                <button
                  type="button"
                  onClick={() => removeInterest(tag)}
                  aria-label={`Remove ${labelText}`}
                  className="hover:text-red-200"
                >
                  <X className="w-3 h-3" />
                </button>
              )}
            </span>
          );
        })}
      </div>

      {/* Approved categories — card grid, no dropdown */}
      <div className="mb-3">
        <p className="text-xs text-slate-500 mb-2">Select from approved categories:</p>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-2">
          {categories.map((categoryItem, idx) => {
            const catLabel = categoryItem?.name || String(categoryItem);
            const isSelected = selectedIds.has(categoryItem?.id);
            return (
              <button
                type="button"
                disabled={isDisabled}
                key={categoryItem?.id ?? `${catLabel}-${idx}`}
                onClick={() => toggleInterest(categoryItem)}
                className={`text-sm text-left px-3 py-2 rounded-2xl border transition-colors ${
                  isDisabled
                    ? "border-slate-200 bg-slate-100 text-slate-400 cursor-not-allowed"
                    : isSelected
                    ? "border-[#B8860B] bg-[#FBF6E9] text-[#0B1526]"
                    : "border-slate-200 hover:bg-slate-50"
                }`}
              >
                {catLabel}
              </button>
            );
          })}
        </div>
      </div>

      {/* Add a custom interest not in the list */}
      {!isDisabled && (
        <div className="mb-3 border-t border-slate-100 pt-3">
          <p className="text-xs text-slate-500 mb-2">Not listed? Add a custom interest (pending approval):</p>
          <div className="flex gap-2">
            <input
              placeholder="Enter your interest..."
              value={otherInterest}
              onChange={(e) => setOtherInterest(e.target.value)}
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  e.preventDefault();
                  handleAddOther();
                }
              }}
              className="w-full rounded-lg border border-slate-200 text-sm py-2.5 px-3"
            />
            <button
              type="button"
              onClick={handleAddOther}
              className="inline-flex items-center gap-2 bg-[#B8860B] text-white px-3 py-1.5 rounded-md text-sm whitespace-nowrap"
            >
              <Send className="w-4 h-4" /> Add
            </button>
          </div>
        </div>
      )}
    </div>
  );
}